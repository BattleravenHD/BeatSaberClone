...
public class BeatSlicer : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {            
            ...
            DetirmineSliceSuccsess(other, transform.position, transform.position + transform.up, thirdSlicingPlanePoint);
            SliceTarget(target, transform.position, transform.position + transform.up, thirdSlicingPlanePoint);
            hand.SendHapticImpulse(0.5f, 0.1f);
        }
    }

    void DetirmineSliceSuccsess(Collider other, Vector3 planePointA, Vector3 planePointB, Vector3 planePointC)
    {
        MovingBeat beat = other.GetComponent<MovingBeat>();
        if (beat.isRed != isRed)
        {
            playerStats.FailedSlice();
        }else
        {
            Plane plane = new Plane(planePointA, planePointB, planePointC);
            switch (beat.direction)
            {
                case Direction.None:
                    playerStats.GoodSlice();
                    break;
                case Direction.Up:
                    if (Mathf.Abs(plane.normal.x) > Mathf.Abs(plane.normal.y) && plane.normal.x < 0)
                    {
                        playerStats.GoodSlice();
                    }else
                    {
                ...
                case Direction.Down:
                    ...
                    break;
                case Direction.Left:    
                    if (Mathf.Abs(plane.normal.x) < Mathf.Abs(plane.normal.y) && plane.normal.y < 0)
                    {
                        playerStats.GoodSlice();
                    }
                    else
